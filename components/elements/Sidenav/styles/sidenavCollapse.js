  ...
  return {
    background: () => {
      ...
      if (transparentSidenav && darkMode) {
        ...
        backgroundValue = active ? palette['dark'].main : transparent.main;
      }
...
function collapseIconBox(theme, ownerState) {
  ...
  const { transparentSidenav, whiteSidenav, darkMode, gridOrientation } = ownerState;

  const { white, dark } = palette;
  const { borderRadius } = borders;
  const { pxToRem } = functions;

  // Determine grid-auto-flow based on gridOrientation
  const gridAutoFlow = gridOrientation === 'horizontal' ? 'column' : 'row';
  ...
  return {
    ...
    display: "grid",
    placeItems: "center",
    gridAutoFlow: gridAutoFlow, // Add this line to set the grid orientation
